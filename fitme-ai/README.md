...
- **PWA server actions:** `experimental.serverActions.allowedOrigins` includes `NEXT_PUBLIC_APP_URL` / `BETTER_AUTH_URL` / `VERCEL_URL` hosts so iOS standalone installs are not blocked by Origin/Host CSRF checks.
