...
- **PWA server actions:** `experimental.serverActions.allowedOrigins` includes `null` (opaque iOS PWA origins), `NEXT_PUBLIC_APP_URL` / `BETTER_AUTH_URL` / `VERCEL_URL` / `VERCEL_PROJECT_PRODUCTION_URL` hosts, and optional `SERVER_ACTIONS_ALLOWED_ORIGINS`. Glucose create/edit/delete also use `/api/glucose` so saves are not blocked by Server Action Origin/Host checks.
...
- Every value cites its source: **Known food** or **Estimated** (+ confidence). Estimated items use a distinct amber treatment; loading shows progress steps and tips (Story 2.7 / FR-10 / UX-DR3).
- Unknown foods stay loggable with AI estimates (Story 2.8 / FR-11). Rename to a catalog food to upgrade to **Known food** macros.
